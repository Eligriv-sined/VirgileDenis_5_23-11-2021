  ...
  if (prodLocalStorage === null || prodLocalStorage == 0) {
    ...
    for (let produit in prodLocalStorage) {
      ...
      productArticle.setAttribute(
        "data-id",
        prodLocalStorage[produit].idProduit
      );
...
function supprimerProduit() {
  ...
  for (let a = 0; a < btn_supprimer.length; a++) {
    btn_supprimer[a].addEventListener("click", (event) => {
      ...
      //-----------------------------------------------------//
      prodLocalStorage = prodLocalStorage.filter(
        (e) => e.idProduit !== idsupr || e.couleurProduit !== colorsupr
      );
...
function prixTotal() {
  ...
  for (var i = 0; i < longueurSaisie; ++i) {
    PrixTotal +=
      saisieQuantité[i].valueAsNumber * prodLocalStorage[i].prixProduit;
  }
